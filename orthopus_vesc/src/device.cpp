...
: vescpp::VESCDevice(this_id, comm.get()), can_(std::dynamic_pointer_cast<vescpp::comm::CAN>(comm))
{
  if (!can_)
  {
  ...
  can_->_can_handlers.emplace_back(
    ...
    std::bind(
      &VESCDevice::process_rt_data_downstream, this, std::placeholders::_1, std::placeholders::_2,
      std::placeholders::_3, std::placeholders::_4));
}

void VESCDevice::process_rt_data_downstream(
  vescpp::comm::CAN* can, const vescpp::comm::CAN::Id can_id, const uint8_t data[8],
  const uint8_t len)
{
}

void VESCDevice::send_measures()
{
  RTDataUpstream meas;
